...
public final class MyEasyTrain {
    ...
    public static void fit(
        Trainer trainer, int numEpoch, Dataset trainingDataset, Dataset validateDataset,  List<DurAndMem> durations2  )
        ...
        for (int epoch = 0; epoch < numEpoch; epoch++) {
            ...
            for (Batch batch : trainer.iterateDataset(trainingDataset)) {
                ...
                duration.on();
                trainBatch(trainer, batch, durations2 );
                duration.off();
                durations2.add(duration);
             //   System.out.println( duration.getMem()/1024 + " kiB");
     ...
     */
    public static void trainBatch(Trainer trainer, Batch batch,  List<DurAndMem> durations2  ) {
